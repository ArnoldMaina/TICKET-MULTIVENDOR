		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce-app-h/product_images/".$uniqueImageName)) {
					...
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`,`admin_id`, `product_title`, `product_qty`, `product_price`,`VIP_qty`, `VIP_price`,`regular_qty`, `regular_price`,`children_qty`, `children_price`,`advance_qty`, `advance_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$category_id', '$brand_id','$admin_id', '$product_name', '$product_qty', '$product_price','$VIP_qty', '$VIP_price', '$regular_qty', '$regular_price', '$chidren_qty', '$children_price', '$advance_qty', '$advance_price', '$product_desc', '$uniqueImageName', '$product_keywords')");
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($_FILES['product_image']['name'])) {
		...
		echo json_encode(['status'=> 202, 'message'=> 'Event added']);
		http_response_code($result['status=> 202' ]);
		exit();
