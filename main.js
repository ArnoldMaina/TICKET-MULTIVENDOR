$(document).ready(function(){
	...
	*/
	$("body").delegate(".qty5","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var pprice = row.find('.pprice').val();
		var qty5 = row.find('.qty5').val();
		var total5 = pprice * qty5;
		row.find('.total5').val(total5);
		var net_total=0;
	...
	$("body").delegate(".qty1","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var vprice = row.find('.vprice').val();
		var qty1 = row.find('.qty1').val();

		var total1 = vprice * qty1;
		row.find('.total1').val(total1);
		var net_total=0;
	...
	$("body").delegate(".qty2","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var rprice = row.find('.rprice').val();
		var qty2 = row.find('.qty2').val();

		var total2 = rprice * qty2;
		row.find('.total2').val(total2);
		var net_total=0;
	...
	$("body").delegate(".qty3","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var cprice = row.find('.cprice').val();
		var qty3 = row.find('.qty3').val();
	
		var total3 = cprice * qty3;
		row.find('.total3').val(total3);
		var net_total=0;
	...
	$("body").delegate(".qty4","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var aprice = row.find('.aprice').val();
		var qty4 = row.find('.qty4').val();

		var total4 = aprice * qty4;
		row.find('.total4').val(total4);
		var net_total=0;
	...
	$("body").delegate(".qty",".qty1",".qty2",".qty3",".qty4",".qty5","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var row = $(this).parent().parent();
		var pprice = row.find('.pprice').val();
		...
		var total6 =total1 + total2+ total3+ total4+ total5;
		row.find('.total6').val(total6);		
	...
	$("body").delegate(".total1",".total2",".total3",".total4",".total5","keyup",function(event){
		event.preventDefault();
		var row = $(this).parent().parent();
		var total1 = row.find('.total1').val();
		var total2= row.find('.total2').val();
		var total3 = row.find('.total3').val();
		var total4 = row.find('.total4').val();
		var total5 = row.find('.total5').val();
		var ttotal = total4 + total3 + total2 + total1 + total5;
		row.find('.total6').val(total6);
		var net_total=0;
		$('.total6').each(function(){
			net_total += ($(this).val()-0);
	...
	*/
	$("body").delegate(".update","click",function(event){
		...
		var qty1 = update.find(".qty1").val();
		var qty2 = update.find(".qty2").val();
		var qty3 = update.find(".qty3").val();
		var qty4 = update.find(".qty4").val();		
		var qty5 = update.find(".qty5").val();			
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateCartItem:1,update_id:update_id,qty:qty,qty1:qty1,qty2:qty2,qty3:qty3,qty4:qty4,qty5:qty5},
			success	:	function(data){
				$("#cart_msg").html(data);
				checkOutDetails();
			}
	...
	*/
	function net_total(){
		...
		$('.qty','.qty1','.qty2').each(function(){
			...
			var ttotal =total1 + total2+ total3+ total4+ total5;//price
			row.find('.ttotal').val(ttotal);
		})
		$('.total6').each(function(){
			net_total += ($(this).val()-0);
		})
		$('.net_total').html("Total : "+ CURRENCY+ " " +total);
	}
